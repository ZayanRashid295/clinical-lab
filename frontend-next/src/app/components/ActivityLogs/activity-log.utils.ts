...
export function getDateRangeForPreset(preset: QuickFilterPreset): {
  dateFrom?: string;
  dateTo?: string;
  component?: string;
} {
  ...
  switch (preset) {
    ...
    case "week": {
      ...
      return { dateFrom: toIsoDate(weekAgo), dateTo: toIsoDate(today) };
    }
    case "auth":
    ...
    case "qbank":
      return { component: "qbank" };
    default:
      return {};
  }
}

export function detectActivePreset(filters: {
  component?: string;
  dateFrom?: string;
  dateTo?: string;
}): QuickFilterPreset {
  ...
  if (filters.component === "auth") return "auth";
  if (filters.component === "assessment") return "assessment";
  if (filters.component === "qbank") return "qbank";
  if (filters.dateFrom === today && filters.dateTo === today) return "today";
  if (filters.dateFrom === weekStart && filters.dateTo === today) return "week";
  return "all";
}
