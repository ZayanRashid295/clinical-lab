...
const RefDesign = () => {
  ...
  const toggleExpand = (id: string) => {
    ...
    const newClosing = new Set(closingItems);

    if (newExpanded.has(id)) {
      // Start closing animation
      newClosing.add(id);
      setClosingItems(newClosing);

      // Remove from expanded after animation completes
      setTimeout(() => {
        newExpanded.delete(id);
        setExpandedItems(new Set(newExpanded));
        newClosing.delete(id);
        setClosingItems(new Set(newClosing));
      }, 500);
    } else {
      // Opening - remove from closing state if it was there
      newClosing.delete(id);
      setClosingItems(newClosing);
      newExpanded.add(id);
