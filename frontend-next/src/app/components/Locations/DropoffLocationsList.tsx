...
interface DropoffLocationsListProps {
  onLocationSelect?: (location: LocationData) => void;
  selectedLocationId?: string;
}

const DropoffLocationsList: React.FC<DropoffLocationsListProps> = ({
  onLocationSelect,
  selectedLocationId,
}) => {
  ...
  // Filter to only show dropoff locations
  ...
  const filteredLocations = dropoffLocations.filter((location) => {
    const matchesSearch =
  ...
  const categories = Array.from(
    new Set(
      dropoffLocations.map((loc) => loc.metadata?.category).filter(Boolean)
    )
  );

  const totalLocations = dropoffLocations.length;
