...
export function StatCard({ title, value, subtitle, icon: Icon, progress, color = "primary" }: StatCardProps) {
  ...
  return (
    <Card data-testid={`card-stat-${title.toLowerCase().replace(/\s+/g, '-')}`} className="bg-white dark:bg-white/5 dark:backdrop-blur-md border border-gray-200 dark:border-white/10">
