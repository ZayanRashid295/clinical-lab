      ...
      <CardContent className="pt-6">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-gray-900 dark:text-gray-200">Systems</h3>
              ...
              {systemGroups.map((group, idx) => (
                <AccordionItem key={idx} value={`item-${idx}`} className="border border-gray-200 dark:border-gray-700 rounded-lg px-3">
                  <AccordionTrigger className="hover:no-underline py-3" data-testid={`accordion-${group.category.toLowerCase().replace(/\s+/g, '-')}`}>
                    <span className="font-medium text-sm text-gray-900 dark:text-gray-200">{group.category}</span>
                  </AccordionTrigger>
                  <AccordionContent>
                      ...
                      {group.systems.map((system) => (
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            ...
                            />
                            <Label htmlFor={system.id} className="cursor-pointer font-normal text-sm text-gray-900 dark:text-gray-200">
                              {system.name}
                            </Label>
                          </div>
