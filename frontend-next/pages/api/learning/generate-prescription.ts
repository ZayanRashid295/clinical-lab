...
import { handleGeneratePrescription } from "@/lib/medprep-shadow/shadow-llm-handlers"
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const out = await handleGeneratePrescription(body)
    return res.status(200).json(out)
  } catch (e) {
    console.error("generate-prescription:", e)
