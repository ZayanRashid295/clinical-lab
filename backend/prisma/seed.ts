...
async function main() {
  ...
  await prisma.address.create({
    data: {
      ...
      name: "Home",
      address: "100 Residential St, Suburb, City, State 12345",
      latitude: 40.75,
      longitude: -73.95,
      city: "New York",
      state: "NY",
  ...
  await prisma.address.create({
    data: {
      ...
      name: "Work",
      address: "200 Business Ave, Downtown, City, State 12345",
      latitude: 40.72,
      longitude: -74.0,
      city: "New York",
      state: "NY",
  ...
  const ride1 = await prisma.ride.create({
    data: {
      ...
      pickupLocationId: createdLocations[0].id,
      dropoffLocationId: createdLocations[1].id,
  ...
  const ride2 = await prisma.ride.create({
    data: {
      ...
      pickupLocationId: createdLocations[1].id,
      dropoffLocationId: createdLocations[2].id,
      pickupLatitude: 40.6892,
      pickupLongitude: -74.1745,
  ...
  await prisma.payment.create({
    data: {
      ...
      method: "CARD",
      gateway: "STRIPE",
      description: "Ride payment for trip #2",
    },
  });

  // Create ride reviews
  console.log("⭐ Creating ride reviews...");
  await prisma.rideReview.create({
    data: {
      rideId: ride1.id,
      reviewerId: passenger1.id,
      revieweeId: driver1.id,
      rating: 5,
  ...
  await prisma.rideReview.create({
    data: {
      rideId: ride1.id,
      reviewerId: driver1.id,
      revieweeId: passenger1.id,
      rating: 5,
      comment: "Excellent passenger, on time and polite.",
    },
  });
  ...
  await prisma.notification.create({
    data: {
      ...
      message: "Your ride has been completed. Thank you for choosing us!",
      data: {
        rideId: ride1.id,
        fare: 15.5,
      },
  ...
  // Create payout fees
  console.log("💸 Creating payout fees...");
  await prisma.payoutFee.create({
    data: {
  ...
  await prisma.payoutFee.create({
    data: {
      ...
      type: "PERCENTAGE",
      value: 0.0, // Base value for percentage calculation
      percentage: 0.029,
      isActive: true,
      appliesTo: ["PAYPAL"],
    },
  ...
  await prisma.payoutFee.create({
    data: {
      ...
      percentage: 0.025,
      isActive: true,
      appliesTo: ["STRIPE_CONNECT"],
    },
  });

  // Create payout settings for drivers
  console.log("⚙️ Creating payout settings...");
  ...
  await prisma.earnings.create({
    data: {
      ...
      rideId: ride1.id,
      grossEarnings: 15.5,
      platformFee: 2.33, // 15% platform fee
      netEarnings: 13.17,
      status: "PENDING",
    },
  });
  ...
  const payout1 = await prisma.payout.create({
    data: {
      ...
      scheduledAt: new Date("2024-01-14T00:00:00Z"),
      processedAt: new Date("2024-01-14T08:30:00Z"),
    },
  });

  // Create payout method details
  await prisma.payoutMethodDetails.create({
    data: {
      payoutId: payout1.id,
      method: "BANK_TRANSFER",
      provider: "bank",
      providerId: "bank_account_123456",
      accountDetails: {
        accountNumber: "****1234",
        routingNumber: "****5678",
        bankName: "Chase Bank",
  ...
  // Create payout items for the payout
  ...
  const payout2 = await prisma.payout.create({
    data: {
      ...
      status: "PENDING",
      payoutMethod: "PAYPAL",
      description: "Bi-weekly payout for completed rides",
      scheduledAt: new Date("2024-01-20T00:00:00Z"),
    },
  });

  // Create payout schedule for driver1
  await prisma.payoutSchedule.create({
    data: {
      ...
      minimumAmount: 50.0,
      isActive: true,
      lastProcessedAt: new Date("2024-01-14T08:30:00Z"),
      nextScheduledAt: new Date("2024-01-21T00:00:00Z"),
    },
  });

  // Create payout schedule for driver2
  await prisma.payoutSchedule.create({
    data: {
      driverId: driver2.id,
      frequency: "BIWEEKLY",
      ...
      minimumAmount: 100.0,
      isActive: true,
      nextScheduledAt: new Date("2024-01-29T00:00:00Z"),
    },
  });
