...
export class NotificationsScheduler {
  ...
  /**
   * SUBSCRIPTION_EXPIRING — daily at 09:00. Notify users whose subscription
   * ends in 7, 3, or 1 days. SUBSCRIPTION_EXPIRED for ones that just lapsed.
   */
  @Cron("0 9 * * *", { name: "subscription:expiry" })
  async runSubscriptionExpiry(): Promise<void> {
    ...
    for (const days of horizons) {
      ...
      for (const sub of subs) {
        const dedupeKey = `sub:${sub.id}:expiring:${days}d`;
        ...
        if (recent.some((n) => (n.data as any)?.dedupeKey === dedupeKey))
          continue;
        ...
        await this.notifications.emit({
          ...
          message: sub.subscriptionPackage?.name
            ? `Renew "${sub.subscriptionPackage.name}" to keep full access.`
            : "Renew your plan to keep full access.",
          data: { subscriptionId: sub.id, daysUntilExpiry: days, dedupeKey },
        });
        sent++;
      }
    }

    // Just-expired subscriptions
    ...
    for (const sub of expired) {
      const dedupeKey = `sub:${sub.id}:expired`;
      const recent = await this.prisma.notification.findMany({
        where: { userId: sub.userId, type: "SUBSCRIPTION_EXPIRED" },
        orderBy: { createdAt: "desc" },
        take: 30,
        select: { data: true },
      });
      ...
      await this.notifications.emit({
        ...
        data: { subscriptionId: sub.id, dedupeKey },
      });
      sent++;
    }
